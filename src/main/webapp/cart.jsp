                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                            ...
                            <div class="lg:col-span-1">
                                <div
                                    ...
                                    <div class="mb-6">
                                        <c:choose>
                                            <c:when test="${not empty sessionScope.selectedVoucher}">
                                                <div
                                                    class="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl p-3">
                                                    <span
                                                        class="material-symbols-outlined text-green-500 text-xl shrink-0">check_circle</span>
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-xs font-bold text-green-700 truncate">
                                                            ${sessionScope.selectedVoucher.code}</p>
                                                        <p class="text-[11px] text-green-600 truncate">
                                                            ${sessionScope.selectedVoucher.title}</p>
                                                    </div>
                                                    <form action="${pageContext.request.contextPath}/voucher"
                                                        method="post">
                                                        <input type="hidden" name="action" value="remove" />
                                                        <button type="submit"
                                                            class="text-xs text-red-400 hover:text-red-600 font-bold">✕</button>
                                                    </form>
                                                </div>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="${pageContext.request.contextPath}/voucher"
                                                    class="flex items-center gap-2 w-full bg-primary/5 border-2 border-dashed border-primary/30 rounded-xl p-3 hover:bg-primary/10 hover:border-primary/60 transition-all group">
                                                    <span class="material-symbols-outlined text-primary">sell</span>
                                                    <span class="text-sm font-semibold text-primary flex-1">Chọn mã giảm
                                                        giá</span>
                                                    <span
                                                        class="material-symbols-outlined text-primary/60 text-base group-hover:translate-x-1 transition-transform">arrow_forward</span>
                                                </a>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                    ...
                                    <div class="flex justify-between items-end mb-6">
                                        ...
                                        <div class="text-right">
                                            <c:if test="${not empty sessionScope.selectedVoucher}">
                                                <c:set var="discountLine"
                                                    value="${totalPrice * sessionScope.selectedVoucher.discountValue / 100}" />
                                                <c:if test="${sessionScope.selectedVoucher.type == 'FIXED'}">
                                                    <c:set var="discountLine"
                                                        value="${sessionScope.selectedVoucher.discountValue}" />
                                                </c:if>
                                                <p class="text-xs text-green-600 font-bold mb-0.5">-
                                                    <fmt:formatNumber value="${discountLine}" type="number"
                                                        groupingUsed="true" />đ voucher
                                                </p>
                                            </c:if>
