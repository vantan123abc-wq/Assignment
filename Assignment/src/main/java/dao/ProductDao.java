...
public class ProductDao {
        ...
        public Product findById(int id) {
                String query = "SELECT id, category_id, name, price, stock as quantity, description, image_url, discount_percent FROM Product WHERE id = ?";
        ...
        public boolean update(Product p) {
                String query = "UPDATE Product SET category_id = ?, name = ?, price = ?, stock = ?, description = ?, image_url = ?, discount_percent = ? WHERE id = ?";
                try (Connection conn = DBConnection.getConnection();
                                PreparedStatement ps = conn.prepareStatement(query)) {
                        ps.setInt(1, p.getCategoryId());
                        ps.setString(2, p.getName());
